...
const Model = memo(function Model({ url, meshStates, onMeshesDetected, decals, selectedDecalId, setSelectedDecalId, updateDecal, removeDecal, finish, globalPattern, mouseFollow }) {
  ...
  useEffect(() => {
    ...
    const finalMeshInfo = meshes.map(m => {
      ...
      try {
        if (m.material) {
          if (Array.isArray(m.material) && m.material.length > 0) {
            if (m.material[0].color && typeof m.material[0].color.getHexString === 'function') {
              originalHex = `#${m.material[0].color.getHexString()}`;
            }
          } else if (m.material.color && typeof m.material.color.getHexString === 'function') {
            originalHex = `#${m.material.color.getHexString()}`;
          }
        }
      } catch (e) {
        console.warn("Could not extract material color for mesh:", m.name);
      }
      return { id: m.name, display: finalName, originalColor: originalHex };
    });
    if (finalMeshInfo.length > 0) {
